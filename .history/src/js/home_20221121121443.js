 ...
 function Vieo_detail(data) {
   ...
   if (ListVideouser) {
     ...
     for (let item of data) {
       ListVideouser.innerHTML += `
              <div class="video_user_detail" onclick="clikvideoview()">
...
// click ++value view 
  var clickvideoid = document.querySelector(".video_user_detail");
  function clikvideoview(){
    alert("Hello world");
  }
      var checkclickvideoid = document.querySelector(".video_user_detail");
  if (checkclickvideoid.clicked == true) {
    var valueviewvideo = 0;
    var valuespan = document.querySelectorAll(".view_video");
    function inraview(){
    for (let j = 0; j < valuespan.length; j++) {

        console.log(valuespan[j]);
        valueviewvideo++;
        valuespan[j].innerHTML = valueviewvideo;
      }
    }
    inraview();
  };
