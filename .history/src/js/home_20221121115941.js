...
if (valuespan){
  ...
  function numberview() {
    for (let i = 0; i < valuespan.length; i++) {
      console.log(valuespan[i]);
      valueviewvideo++;
      valuespan[i].innerHTML = valueviewvideo;
    }
  }
}
if (numberview()){
  numberview()var clickvideoid = document.querySelectorAll(".video_user_detail");
for (let i = 0; i < clickvideoid.length; i++) {
  valuespan[i].addEventListener("click", () => {
